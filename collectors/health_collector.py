...
class HealthCollector(object):
    ...
    def parse_nvme_info(self, providing_drives):
        ...
        info_labels = {"disk": f"/dev/{disk_name}", "type": providing_drives.get("MediaType", "").lower(),"serial_number": attributes.get("Serial number", ""),"model_family": attributes.get("Device model", "").lower(),"host":self.col.host }
        ...
        power_cycle = attributes.get("Power_Cycle_Count", 0)
        if  power_cycle.isdigit():
        ...
        power_on_hours = attributes.get("Power on hours", 0)
        if  power_on_hours.isdigit():
        ...
        media_errors = attributes.get("Media errors", 0)
        if media_errors.isdigit():
